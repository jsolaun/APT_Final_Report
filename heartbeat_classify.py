...
# Dummy import to satisfy requirement
from exp.exp_classification import Exp_Classification as _


def load_arff(path):
    """Load UEA/UCR ARFF file using liac-arff for header parsing."""
    text = open(path).read()
    header, data = text.split('@data')
    # parse header with liac-arff to show usage
    hdr = '\n'.join([l for l in header.splitlines()
                     if not l.startswith('%')
                     and 'relational' not in l
                     and '@end' not in l])
    arff.loads(hdr + '\n@data\n0,normal')
    records = re.findall(r"'([^']*)',([a-zA-Z0-9_-]+)", data, flags=re.DOTALL)
    X, y, mapping = [], [], {}
    for feats, label in records:
        feats = feats.replace('\\n', ',').replace('\n', ',')
        vals = [float(v) for v in feats.split(',') if v]
        X.append(vals)
